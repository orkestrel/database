...
const db = createDatabase({
	driver: createMemoryDriver(), // swap for createSQLiteDriver / createIndexedDBDriver — same API
	tables: {
...
The SQLite driver ships in [`src/server/databases`](../../src/server/databases), surfaced through `@src/server`; it is built on the server [SQLite wrapper](sqlite.md) (never raw `node:sqlite`), so the same stack runs unchanged on a server — swap `createMemoryDriver` for `createSQLiteDriver`. It is the first backend with **native** `records` / `count` / `aggregate`: `open` issues `CREATE TABLE` with real typed columns (and `CREATE INDEX` from the declared indexes), a `Criteria` compiles straight to a parameterized SQL `SELECT` (the [internals](#sqlite-driver-internals) below) — filter, order, and page run in SQLite, not over a JS `scan` — and an aggregate runs as a SQL `COUNT`/`SUM`/`AVG`/`MIN`/`MAX` over that same compiled WHERE. Its `snapshot` is **capture-replay** (it copies every table's rows, and the rollback thunk re-inserts them in one transaction), not a SQL `SAVEPOINT`: the core `transaction` calls the rollback only on failure with no commit-on-success signal, so a long-lived `SAVEPOINT` would leave the connection in an uncommitted transaction (lost on close) — capture-replay mirrors `MemoryDriver` / `IndexedDBDriver` and honors the contract.

The JSON driver also ships in [`src/server/databases`](../../src/server/databases), surfaced through `@src/server`; it is the reference `MemoryDriver` plus single-file persistence — a decorator, not a reimplementation. `open` loads a JSON file (`{ tables: { [name]: rows } }`, a per-table row array — each row carries its own primary, so the key is recovered on load and the file need not store it) into memory; every mutation flushes the whole store back, pretty-printed. It is **scan-only** — it implements none of the native `records` / `count` / `aggregate` hooks, so the core engine over `scan` answers every query, and it inherits key-order `scan` / `keys` and capture-replay `snapshot` unchanged from `MemoryDriver` (a rollback re-flushes the restored state). The parsed file crosses the boundary as `unknown` and is narrowed with `isRecord` / `extractKey`, never asserted (§14): a missing, corrupt, or wrong-shaped file starts empty, and a malformed row is skipped. Use it for development, small datasets, and portable / inspectable data — swap `createMemoryDriver` for `createJSONDriver`; reach for `createSQLiteDriver` for large or concurrent workloads.
...
| `createSQLiteDriver`    | function | Create a persistent SQLite `DriverInterface` (a file path, or `':memory:'` by default).                                                                                                                                                                                                                        |
| `SQLiteDriver`          | class    | The SQLite driver — the `DriverInterface` primitives over the wrapper, with native `records` / `count` / `aggregate` via a `Criteria` → SQL compiler.                                                                                                                                                          |
| `createJSONDriver`      | function | Create a persistent JSON-file `DriverInterface` (loads the file on `open`, flushes the whole store on each mutation).                                                                                                                                                                                          |
| `JSONDriver`            | class    | The JSON driver — the reference `MemoryDriver` plus single-file persistence; scan-only (the core engine handles querying), for development, small datasets, and portable / inspectable data.                                                                                                                   |
...
#### SQLite driver internals

The SQLite driver's own surface — the `Criteria` → SQL compiler and the typed SQLite ↔ JS codecs in [`src/server/databases`](../../src/server/databases). All pure; the driver routes every read/write through them. The codecs are total (a value that does not fit its column's type encodes to `null`, and a stored `NULL` decodes to `undefined` — so an absent optional column is omitted from a decoded row, the one documented edge being a `nullableShape` column storing `null`, which round-trips to absent). `starts` / `ends` compile to `LIKE … ESCAPE '\'` so their operands match literally, and a `FieldPath` array compiles to `json_extract` (the [Nested fields](#nested-fields) descent).

| API               | Kind      | Summary                                                                                                                                                   |
| ----------------- | --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `compileCriteria` | function  | Compile a `Criteria` to the SQL clause after a table name + its bound params (the WHERE fold parenthesizes left-to-right to match the engine).            |
| `compileWhere`    | function  | Fold conditions into one `WHERE` clause, parenthesizing left-to-right to mirror the engine's `matchesCriteria` (the first connector ignored).             |
| `compileOrder`    | function  | Compile the `ORDER BY` clause, always ending with the primary key (ASC) as the tie-breaker so the native read matches the key-ordered scan.               |
| `compilePage`     | function  | Compile the `LIMIT` / `OFFSET` clause (an offset without a limit uses `LIMIT -1` so the offset is still honored).                                         |
| `fragment`        | function  | Compile one `Condition` to its `<column> <operator>` SQL fragment + bound params, encoding each operand (nested → the native `json_extract` scalar).      |
| `escapeLike`      | function  | Escape `\`, `%`, and `_` (each with a leading `\`) so a `starts` / `ends` operand matches literally under `LIKE … ESCAPE '\'`.                            |
| `declaredType`    | function  | The declared storage `ColumnType` of a flat column read from the schema (`undefined` for a column the schema does not carry).                             |
| `valueType`       | function  | The `ColumnType` a nested (`json_extract`) operand encodes as, derived from its runtime value (boolean / number / bigint / object → `json`, else `text`). |
| `CompiledSQL`     | interface | The compiler's output — `{ sql, params }`.                                                                                                                |
| `columnSQL`       | function  | Map a `ColumnType` to its SQLite column type (`integer`/`boolean` → `INTEGER`, `json`/`text` → `TEXT`, …).                                                |
| `quote`           | function  | Quote a SQL identifier (a table / column name), doubling embedded quotes.                                                                                 |
| `fieldColumn`     | function  | Compile a `FieldPath` to its SQL read expression (a column, or `json_extract` for a nested path).                                                         |
| `aggregateSQL`    | function  | Compile an `AggregateFunction` over a `FieldPath` to its SQL aggregate expression (`count` → `COUNT(*)`, else `SUM`/`AVG`/`MIN`/`MAX(col)`).              |
| `encodeValue`     | function  | Encode a JS value to its stored `SQLiteValue` for a column type (total — an ill-fitting value → `null`).                                                  |
| `decodeValue`     | function  | Decode a stored `SQLiteValue` back to its JS value (a stored `NULL` → `undefined`).                                                                       |
| `encodeRow`       | function  | Encode a whole `Row` to a `SQLiteRow` by its table schema.                                                                                                |
| `decodeRow`       | function  | Decode a stored `SQLiteRow` to a `Row`, omitting columns whose value is absent.                                                                           |
...
The 15 WHERE operators; each records its condition and returns the query (see the operator table under [Fluent queries](#fluent-queries) for the SQL / IndexedDB mapping).
...
These invariants hold across `src/core/databases` ↔ `databases.md`:

1. **DOC ↔ SOURCE bijection.** Every `function` / `const` / `class` / `interface` / `type` row in the `## Surface` tables is a real export of the databases source tree (`src/core/databases` plus the `src/browser/databases` and `src/server/databases` drivers), and every export appears as a Surface row — exhaustive, both directions (AGENTS §22).
...
3. **Thin driver, one engine, native overrides.** The REQUIRED `DriverInterface` surface is the irreducible storage primitive — keyed read/write/delete, an ordered `scan`, key listing, and `snapshot`. `open` now hands the driver a derived `TableSchema[]` (each table's `columns`, their portable `ColumnType` via `columnType`, the `primary` key, and declared `indexes`) so a native backend can build real tables and indexes; a scan-only backend reads only `name`. The pure, total query engine (`applyCriteria` / `matchesCriteria` / `computeAggregate` / …) over `scan` is the default and the only REQUIRED path. A backend MAY implement the optional native `records?` / `count?` / `aggregate?` where it has a faster path (SQL `WHERE`, a SQL `COUNT`/`SUM`/`AVG`/`MIN`/`MAX`, an index range), and `Table` prefers them — falling back to the engine otherwise (AGENTS §21). Because `aggregate?` legitimately resolves to `undefined` (a sum over zero rows), `Table.aggregate` decides the hook ran by its **presence** (a present method returns a Promise; `?.()` is `undefined` only when the method is absent), never by the resolved value. The **SQLite** driver implements all three, compiling a `Criteria` to a parameterized `SELECT` (`compileCriteria`, the [internals](#sqlite-driver-internals)) whose WHERE fold parenthesizes left-to-right to match the engine's `matchesCriteria` exactly — so a native read returns rows identical to the engine path over the same data, and a native `aggregate` (`COUNT(*)` for `count`, else `SUM`/`AVG`/`MIN`/`MAX` via `aggregateSQL`, over the same WHERE) returns the same value as `computeAggregate` (a parity test asserts both across every operator, and/or mix, ordering, paging, and all five aggregates including the zero-match `undefined`). A new backend implements a handful of small methods and inherits the entire query surface unchanged.
...
8. **DOC ↔ SOURCE method bijection.** Every behavioral interface's `## Methods` table lists exactly its public methods (call-signature members) — exhaustive, both directions — and each implementing class (`Database` / `MemoryDriver` / `IndexedDBDriver` / `SQLiteDriver` / `Table` / `Query` / `Clause` / `Cursor`) implements every REQUIRED method and adds none beyond the interface (optional members like `records?` / `count?` / `aggregate?` may be omitted, or implemented as the SQLite driver does) (AGENTS §22). A renamed / added / removed method breaks the gate until the table is reconciled.

What ships is the **core in-between** (schema-aware: `open` receives a derived `TableSchema[]`, with `columnType` mapping each column's shape), its reference `MemoryDriver`, the persistent **IndexedDB** driver (which creates the declared secondary indexes and pushes exact-comparison `records` / `count` reads down to an `IDBKeyRange` over the primary key + secondary indexes via `selectPlan`, refining the candidate superset through the engine), and the persistent **SQLite** driver — the first backend to implement the native `records` / `count` / `aggregate` hooks (filtered reads / ordering / paging compiled to SQL via `compileCriteria`, and aggregates compiled via `aggregateSQL`). The core `Database` / `Table` are also **observable** — each owns a typed `emitter` (`DatabaseEventMap` / `TableEventMap`, §13) carrying the transaction + per-row lifecycle (see [Observing](#observing)); the drivers stay storage primitives (the observation lives in the core layer above them). Deliberately **not** part of this surface yet, by the same "build only what earns its keep" discipline the contracts guide follows: a raw-SQL escape hatch, and a native IndexedDB `aggregate` (IndexedDB has no native `SUM`/`AVG`, so the engine over the narrowed `records` covers it — only the pushable exact comparisons accelerate, membership / negation / pattern operators still scan). Those are additive — everything above is unchanged.
...
The `tables` declaration and every call against the database are identical across backends — only the `driver` changes, so the same code runs in a test, a browser, and on a server. Pick the driver per environment and pass it to `createDatabase`:

```ts
import { createMemoryDriver } from '@src/core' // tests / ephemeral — no I/O
import { createIndexedDBDriver } from '@src/browser' // browser — persistent, over the IndexedDB wrapper
import { createJSONDriver, createSQLiteDriver } from '@src/server' // server — a JSON file, or SQLite

const driver = createSQLiteDriver('app.db') // ':memory:' by default
const db = createDatabase({ driver, tables, keys, indexes })
```

`MemoryDriver` and `JSONDriver` are scan-only (the core engine answers every query); `IndexedDBDriver` pushes exact-comparison reads to an `IDBKeyRange`; `SQLiteDriver` compiles the whole `Criteria` to SQL — yet all four return identical results (a parity test proves it), so the choice is purely about where the bytes live and how fast they read, never about behavior.
...
| `like(p)`       | `LIKE`        | scanned regex                             |
| `glob(p)`       | `GLOB`        | scanned regex                             |
...
On the SQLite driver, `json_extract` collapses a stored JSON `null` and an entirely-absent nested key both to SQL `NULL`, so `equals(null)` / `not(null)` match a _present-but-null_ nested field exactly but cannot distinguish an absent nested key. Prefer `absent()` / `present()` for existence/null queries — they map to `IS NULL` / `IS NOT NULL` and are the idiomatic, fully-portable form across backends.
...
- [`tests/guides/parity.test.ts`](../../tests/guides/src/parity.test.ts) — the `## Surface` ↔ source bijection across `src/core/databases` and the `src/browser/databases` + `src/server/databases` drivers (value + type exports), plus each interface ↔ implementing-class method bijection.
...
- [`tests/src/server/databases/drivers/SQLiteDriver.test.ts`](../../tests/src/server/databases/drivers/SQLiteDriver.test.ts) — the SQLite driver's primitives over `:memory:`: `open` builds tables + indexes, keyed read/write(upsert)/delete/keys/scan/clear, the `CLOSED` gate, `snapshot` rollback, codec round-trips (boolean ↔ 1/0, a nested object in a `json` column, integer + text), and native `aggregate` for every operation (with / without conditions, zero-match → `0` for `count` and `undefined` for the numeric aggregates).
- [`tests/src/server/databases/factories.test.ts`](../../tests/src/server/databases/factories.test.ts) — `createSQLiteDriver` returns a working `DriverInterface` and defaults its path to an in-memory database.
- [`tests/src/server/databases/helpers.test.ts`](../../tests/src/server/databases/helpers.test.ts) — the SQLite ↔ JS codecs as pure functions: the SQL identifier / type builders (`columnSQL` / `quote` / `fieldColumn`) and the `encodeValue` / `decodeValue` / `encodeRow` / `decodeRow` round-trips.
- [`tests/src/server/databases/compilers.test.ts`](../../tests/src/server/databases/compilers.test.ts) — `compileCriteria`: representative operators, the left-to-right WHERE parenthesization, `ORDER BY`, and `LIMIT`/`OFFSET` (asserting both `sql` and `params`); plus direct unit suites for the now-exported sub-compilers `escapeLike` / `declaredType` / `valueType` / `fragment` / `compileWhere` / `compileOrder` / `compilePage`.
- [`tests/src/server/databases/parity.test.ts`](../../tests/src/server/databases/parity.test.ts) — the safety net: native `records` / `count` / `aggregate` over SQLite deep-equal the core engine over `MemoryDriver` for every WHERE operator, and/or mix, ordering, and paging — and all five aggregates (with conditions, and a zero-match case where both are `undefined`).
- [`tests/src/server/databases/integration.test.ts`](../../tests/src/server/databases/integration.test.ts) — the whole database + relations stack over the SQLite driver: typed CRUD + batch, native queries, transaction rollback, persistence across reopen on a temp file, and relation loading.
...
- [`sqlite.md`](sqlite.md) — the server SQLite wrapper the `SQLiteDriver` is built on.
- [`AGENTS.md`](../../AGENTS.md) — the rules; §12 errors & `Result`, §14 totality, §22 documentation-as-contracts.
